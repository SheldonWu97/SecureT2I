...
class Asyrp(object):
    ...
    def run_training(self):
        ...
        if self.args.attack:
            ...
            save_name = './checkpoint/pretrained_'+self.args.edit_attr+'.pth' ###
            print(f'Reload model from: {save_name}.')
            model.module.load_state_dict(torch.load(save_name))
            pre_model = deepcopy(model.module)
            ori_model.eval()
            pre_model.eval()
        ...
        for it_out in range(self.args.start_iter_when_you_use_pretrained ,self.args.n_iter):
            ...
            if self.args.do_train:
                ...
                if self.args.retrain==0 and os.path.exists(save_name):
                    ...
                    for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                        ...
                        with tqdm(total=len(seq_train), desc=f"training iteration") as progress_bar:
                            for t_it, (i, j) in enumerate(zip(reversed(seq_train), reversed(seq_train_next))):
                                ...
                                if self.args.attack:
                                    if step < 50: #forget set
                                        x0 = x0.cuda()
                                        if self.args.max_loss:
                                            loss_clip = (2 - clip_loss_func(x0, self.src_txts[0], x0_t, self.trg_txts[0])) / 2
                                            loss_clip = -torch.log(loss_clip)
                                            loss_id = 0#torch.mean(id_loss_func(x0, x0_t))
                                            loss_l1 = nn.MSELoss()(x0, x0_t)
                                            loss = -self.args.clip_loss_w * loss_clip - self.args.id_loss_w * loss_id - self.args.l1_loss_w * loss_l1
                                        elif self.args.noisy_label:
                                            x_mask = torch.randn_like(x0_t)
                                            loss = nn.MSELoss()(x_mask, x0_t)
                                        elif self.args.retain_label:
                                            x_mask = img_lat_pairs_dic['train'][step+50][0].cuda()
                                            loss = nn.MSELoss()(x_mask, x0_t)
                                        else:
                                            x_mask = transform(x0).cuda()
                                            loss = nn.L1Loss()(x_mask, x0_t)
                                        #loss = nn.MSELoss()(x_mask, x)
                                    else: #retrain set
                                        loss = nn.L1Loss()(x_pre, x0_t)
                                        #continue
                                    loss_id, loss_clip = 0,0
                                else:
                                    # L1 loss
                        ...
                        if not self.args.attack and self.args.save_train_image and save_image_iter % self.args.save_train_image_step == 0 and it_out % self.args.save_train_image_iter == 0:
                            self.save_image(x0, step, ori_model, pre_model, model, x_lat_tensor, seq_test, seq_test_next,
                    ...
                    if not self.args.attack and not self.args.re and it_out == self.args.n_iter-1:
        ...
        # ------------------ Test ------------------#
        if self.args.do_test and it_out == self.args.n_iter-1:
            x_lat_tensor = None
            x0_tensor = None
    ...
    @torch.no_grad()
    def save_image(self, x0, step, ori_model, pre_model, model, x_lat_tensor, seq_inv, seq_inv_next,
            ...
            if self.args.pass_editing:
                ...
                for hs_coeff_tuple in hs_coeff:
                    ...
                    for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                        ...
                        if self.args.attack:
                            x_ori, _, _, _ = denoising_step(x_ori, t=t, t_next=t_next, models=ori_model,
                                        logvars=self.logvar,                                
                                        sampling_type=self.args.sample_type,
                                        b=self.betas,
                                        learn_sigma=self.learn_sigma,
                        ...
                        if self.args.attack:
                            ...
                            if step < 50:
                                save_dir = self.args.edit_attr + add_name + '/forget'
                            else:
                                save_dir = self.args.edit_attr + add_name + '/retain'

                            if not os.path.exists(os.path.join('./out', save_dir, 'orig')):
                                os.makedirs(os.path.join('./out', save_dir, 'orig'))
                            tvu.save_image((x0 + 1) * 0.5, os.path.join('./out', save_dir, 'orig',
                                f'{step}_orig_{self.args.edit_attr}.png'))
                            if not os.path.exists(os.path.join('./out', save_dir, 'ckpt')):
                                os.makedirs(os.path.join('./out', save_dir, 'ckpt'))
                            tvu.save_image((x_ori + 1) * 0.5, os.path.join('./out', save_dir, 'ckpt',
                                f'{step}_ckpt_{self.args.edit_attr}.png'))
                            if not os.path.exists(os.path.join('./out', save_dir, 'pre')):
                                os.makedirs(os.path.join('./out', save_dir, 'pre'))
                            tvu.save_image((x_pre + 1) * 0.5, os.path.join('./out', save_dir, 'pre',
                                f'{step}_pre_{self.args.edit_attr}.png'))
                            if not os.path.exists(os.path.join('./out', save_dir, 'unl')):
                                os.makedirs(os.path.join('./out', save_dir, 'unl'))
                            tvu.save_image((x + 1) * 0.5, os.path.join('./out', save_dir, 'unl',
                                f'{step}_unl_{self.args.edit_attr}.png'))
                    
                            if not os.path.exists(os.path.join('./out', save_dir, 'all')):
                                os.makedirs(os.path.join('./out', save_dir, 'all'))
                            tvu.save_image((x0 + 1) * 0.5, os.path.join('./out', save_dir, 'all',
                                f'{step}_orig_{self.args.edit_attr}.png'))
                            tvu.save_image((x_ori + 1) * 0.5, os.path.join('./out', save_dir, 'all',
                                f'{step}_ckpt_{self.args.edit_attr}.png'))
                            tvu.save_image((x_pre + 1) * 0.5, os.path.join('./out', save_dir, 'all',
                                f'{step}_pre_{self.args.edit_attr}.png'))
                            tvu.save_image((x + 1) * 0.5, os.path.join('./out', save_dir, 'all',
                                f'{step}_unl_{self.args.edit_attr}.png'))
                        elif self.args.re:
                            if step < 50:
                                save_dir = self.args.edit_attr + '_/retrain_forget'
                            else:
                                save_dir = self.args.edit_attr + '_/retrain_retain'
                            if not os.path.exists(os.path.join('./out', save_dir)):
                                os.makedirs(os.path.join('./out', save_dir))
                            tvu.save_image((x + 1) * 0.5, os.path.join('./out', save_dir,
                                f'{step}_retrain_{self.args.edit_attr}.png'))
                        #if save_process_delta_h:
                        #    output = torch.cat([x, x0_t], dim=0)
                        #    output = (output + 1) * 0.5
                        #    grid = tvu.make_grid(output, nrow=self.args.bs_train, padding=1)
                        #    tvu.save_image(grid, os.path.join(folder_dir, file_name, f'delta_h_{int(t[0].item())}.png'), normalize=True)
